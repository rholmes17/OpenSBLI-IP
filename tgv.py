...
def tgv(grid, Re, dt, niter, order=4, saveFreq=50, gama=1.4, Minf=0.1, Pr=0.71):
    ...
    # by default the scheme is Central no need to specify the schemes
    mass = "Eq(Der(rho,t), - Skew(rho*u_j,x_j))"
    momentum = ("Eq(Der(rhou_i,t) , - Skew(rhou_i*u_j, x_j)"
                " - Der(p,x_i)  + Der(tau_i_j,x_j))")
    ...
    stress_tensor = ("Eq(tau_i_j, (1.0/Re)*(Der(u_i,x_j) + Der(u_j,x_i)"
    ...
    # Instantiate EinsteinEquation class for
    # expanding the Einstein indices in the equations
    einstein_eq = EinsteinEquation()

    # Expand the simulation equations, for
    # this create a simulation equations class
    simulation_eq = SimulationEquations()

    # Expand mass and add the expanded
    # equations to the simulation equations
    eqns = einstein_eq.expand(
        mass, ndim, coordinate_symbol, substitutions, constants)
    simulation_eq.add_equations(eqns)

    # Expand momentum add the expanded
    # equations to the simulation equations
    eqns = einstein_eq.expand(
        momentum, ndim, coordinate_symbol, substitutions, constants)
    simulation_eq.add_equations(eqns)

    # Expand energy equation add the expanded
    # equations to the simulation equations
    eqns = einstein_eq.expand(
        energy, ndim, coordinate_symbol, substitutions, constants)
    simulation_eq.add_equations(eqns)

    # Expand the constituent relations
    # and them to the constituent relations class
    # Instantiate constituent relations object
    constituent = ConstituentRelations()

    # Expand momentum add the expanded equations to the constituent relations
    eqns = einstein_eq.expand(
        velocity, ndim, coordinate_symbol, substitutions, constants)
    constituent.add_equations(eqns)

    # Expand pressure add the expanded equations to the constituent relations
    eqns = einstein_eq.expand(
        pressure, ndim, coordinate_symbol, substitutions, constants)
    constituent.add_equations(eqns)

    # Expand temperature add the expanded equations to the constituent relations
    eqns = einstein_eq.expand(
        temperature, ndim, coordinate_symbol, substitutions, constants)
    constituent.add_equations(eqns)
    ...
    local_dict = {"block": block,
                  "GridVariable": GridVariable, "DataObject": DataObject}
    ...
    p = ("Eq(GridVariable(p), 1.0/(gama*Minf*Minf)+ (1.0/16.0)"
         " * (cos(2.0*x0)+cos(2.0*x1))*(2.0 + cos(2.0*x2)))")
    ...
    schemes = {}
    # Central scheme for spatial discretisation
    # and add to the schemes dictionary
    cent = Central(order)
    schemes[cent.name] = cent
    # RungeKutta scheme for temporal discretisation
    # and add to the schemes dictionary
    rk = RungeKutta(3)
    schemes[rk.name] = rk
    ...
    kwargs = {'iotype': "Write"}
    h5 = iohdf5(save_every=saveFreq, **kwargs)
    h5.add_arrays(simulation_eq.time_advance_arrays)
    # h5.add_arrays([DataObject('x0'), DataObject('x1'), DataObject('x2')])
    block.setio(copy.deepcopy(h5))
    # set the equations to be solved on the block
    block.set_equations([copy.deepcopy(constituent),
                         copy.deepcopy(simulation_eq), initial])
    ...
    # set the simulation data type, for more information on the datatypes,
    #  see opensbli.core.datatypes
    SimulationDataType.set_datatype(Double)

    # Write the code for the algorithm
    OPSC(alg)

    constants = ['Re', 'gama', 'Minf', 'Pr', 'dt', 'niter', 'block0np0',
                 'block0np1', 'block0np2', 'Delta0block0',
                 'Delta1block0', 'Delta2block0']
    values = [Re, gama, Minf, Pr, dt, niter, grid, grid, grid,
              'M_PI/(block0np0-1)', 'M_PI/(block0np1-1)', 'M_PI/(block0np2-1)']
